...
1. シフト行の「調整相談」ボタンをクリックする
2. 変更事象（例: スタッフ急休、利用者の日時変更）を選択する
3. システムが「誰をどう入れ替えるか（操作列）」と理由文を 1〜3 案を生成する
4. 管理者は生成された説明を確認し、必要に応じて関係者へ共有する
5. 最終反映は既存の「担当者変更」で手動実行する（このフロー単体ではシフト情報のDB更新は行わない＝自動更新はしない）
