...
週間シフトを生成・閲覧し、担当者を変更・担当者を割り当て・キャンセルなどの操作を行う。
...
├────────────┼─────────────┼──────────┼──────────┼────────┼───────────┼───────────────────┤
│ 01/19 (月) │ 10:00-11:00 │ 田中太郎 │ 身体介護 │ 山田   │ scheduled │ [担当者を変更][キャンセル] │
│ 01/19 (月) │ 14:00-15:00 │ 鈴木花子 │ 生活援助 │ 未割当 │ scheduled │ [担当者を割り当て][キャンセル]   │
...
1. 「担当者を変更」ボタンをクリック
2. `ChangeStaffDialog` が開く（現在の情報を表示）
...
1. シフト行の「担当者を変更」または「担当者を割り当て」で `ChangeStaffDialog` を開く
2. ダイアログ内の「調整相談」から `AdjustmentWizardDialog` を開く
3. 事象（例: スタッフ急休）と対象を選択する
4. 事象に応じて提案を生成・表示する（`staff_absence` は理由付き 1〜3 案、その他は候補提示中心）
5. `staff_absence` の候補選択時は、選択結果を `ChangeStaffDialog` の `initialSuggestion` props に注入する
6. 調整相談単体では DB 更新しない（非永続化）。管理者が `ChangeStaffDialog` で「変更」を押したときのみ更新する

※調整相談は説明生成の補助機能。更新操作は既存の担当者を変更/担当者を割り当て（「変更」）に集約する。

### 担当者を割り当て

- 担当者を変更と同様（現在の担当者が「未割当」）
...
└── _components/
    ...
    ├── ChangeStaffDialog/          # 担当者を変更/担当者を割り当てダイアログ（調整相談導線の起点）
    ├── AdjustmentWizardDialog/     # 調整相談ウィザード（提案生成・非永続化）
