...
1. シフト行の「担当者変更」または「割り当て」で `ChangeStaffDialog` を開く
...
5. `staff_absence` の候補選択時は、選択結果を `ChangeStaffDialog.initialSuggestion` に注入する
6. 調整相談単体では DB 更新しない（非永続化）。管理者が `ChangeStaffDialog` で「変更」を押したときのみ更新する

※調整相談は説明生成の補助機能。更新操作は既存の担当者変更/割り当て（「変更」）に集約する。
...
└── _components/
    ...
    ├── ChangeStaffDialog/          # 担当者変更/割り当てダイアログ（調整相談導線の起点）
