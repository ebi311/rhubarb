...
1. 「調整相談」ボタンをクリック
2. 事象と対象を選択する
3. 「誰をどう入れ替えるか（操作列）」と理由文の提案を 1〜3 案を表示する
4. この画面では提案の表示・共有までを行い、DB更新は行わない
5. 反映は既存の担当者変更/割り当て操作で管理者が手動実行する

※「調整相談」ボタンは説明生成のみの補助機能であり、「操作ボタン表示ルール」表の対象外とする。

### AIアシスタント Drawer

1. ヘッダー右側の「AIアシスタント」ボタンをクリック
2. デスクトップでは画面右側に幅 400px の Drawer を表示し、一覧本体は lg:pr-[400px] の右余白で重なりを避ける
3. Drawer を閉じてもコンポーネントは維持されるため、会話履歴を保持したまま再表示できる
4. Escape キーで閉じられるが、IME 変換中の Escape は無視して誤操作を防ぐ
