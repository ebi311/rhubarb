...
- [ ] 2.1. `/admin/weekly-schedules/page.tsx` を作成
- [ ] 2.2. `week` パラメータ省略時は今週の月曜日にリダイレクト
- [ ] 2.3. 不正な `week` パラメータ（月曜日以外、無効な日付）時のエラーハンドリング
- [ ] 2.4. 管理者のみアクセス可能な認可チェック
- [ ] 2.5. ページのテストを作成
