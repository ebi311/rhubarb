...
- [x] 3.1. `BasicScheduleFilterBar` コンポーネントのテストを作成
- [x] 3.2. `BasicScheduleFilterBar` コンポーネントを実装
  - 曜日セレクト
  - 利用者セレクト（契約中のみ）
  - サービス区分セレクト
  - キーワード検索
- [x] 3.3. Storybook ストーリーを作成（通常、選択済み状態）
- [x] 3.4. テストを実行して成功を確認

### ステップ4: 一覧UIコンポーネント（テーブル）

- [x] 4.1. `BasicScheduleTable` コンポーネントのテストを作成
- [x] 4.2. `BasicScheduleTable` コンポーネントを実装
  - テーブル列: 利用者、サービス区分、曜日、時間帯、担当者、備考
  - 曜日のローカライズ表記
  - 時間帯のフォーマット (HH:MM-HH:MM)
- [x] 4.3. 空状態コンポーネントを実装
- [x] 4.4. Storybook ストーリーを作成（通常、空状態、ローディング）
- [x] 4.5. テストを実行して成功を確認

### ステップ5: 一覧ページの統合

- [x] 5.1. `/admin/basic-schedules/page.tsx` に一覧ページのテストを作成
- [x] 5.2. 一覧ページにフィルタバーとテーブルを統合
- [x] 5.3. `fetchBasicSchedules` を実装してデータ取得
- [x] 5.4. フィルタ変更時の再取得ロジックを実装（URLパラメータ連動）
- [x] 5.5. ローディング状態の表示を実装（Suspense）
- [x] 5.6. テストを実行して成功を確認

### ステップ6: 統合テストとStorybookの完成

- [x] 6.1. 一覧ページの全状態のStorybookストーリーを確認
- [x] 6.2. フィルタ連動の統合テストを実行
- [x] 6.3. `pnpm test:ut --run` で全テストが成功することを確認
