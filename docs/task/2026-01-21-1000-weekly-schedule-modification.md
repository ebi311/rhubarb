...
- [ ] **ChangeStaffDialog コンポーネント**
  - [ ] コンポーネント本体作成
    - [ ] シフト情報表示（読み取り専用）
    - [ ] スタッフピッカー統合
    - [ ] 変更理由入力（テキストエリア、任意）
    - [ ] 時間重複チェック（`validateStaffAvailabilityAction` 呼び出し）
    - [ ] StaffConflictWarning 表示
    - [ ] `changeShiftStaffAction` 呼び出し
    - [ ] トースト通知（「〇〇さん → △△さんに変更しました」）
  - [ ] テスト作成
    - [ ] ダイアログの開閉
    - [ ] スタッフ選択とバリデーション
    - [ ] 時間重複チェック
...
- [ ] **CancelShiftDialog コンポーネント**
  - [ ] コンポーネント本体作成
    - [ ] シフト情報表示（読み取り専用）
    - [ ] キャンセル理由カテゴリ選択（ラジオボタン）
    - [ ] キャンセル理由詳細入力（テキストエリア、必須）
    - [ ] 確認ダイアログ
    - [ ] `cancelShiftAction` 呼び出し
    - [ ] トースト通知（「シフトをキャンセルしました」）
  - [ ] テスト作成
    - [ ] ダイアログの開閉
    - [ ] カテゴリ選択とバリデーション
    - [ ] 理由入力とバリデーション
