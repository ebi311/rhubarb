...
- `suggestClientDatetimeChangeAdjustmentsAction` は削除済み（staff_absence 経路のみを維持）

**削除/整理対象サービスメソッド**:

- `shiftAdjustmentSuggestionService.ts` — `suggestShiftAdjustments` と `suggestClientDatetimeChangeAdjustments` の旧公開経路は削除済み（staff_absence 経路のみ）

**受け入れ条件**:

- [x] 削除後もテスト全通過
- [x] 型エラーなし (`pnpm typecheck` でエラーなし)
- [x] `ShiftAdjustmentDialog` へのインポートが残っていない（確認コマンド: `git grep -nE "import .*ShiftAdjustmentDialog" -- src`）

**完了サマリ（2026-03-05 更新）**:

- Phase 4 の実装経路削除（Action/Service）と検証条件の更新を反映。
- `src/models/shiftAdjustmentActionSchemas.test.ts` の `client_datetime_change` 参照を削除し、文言依存アサーションを解消。
- 旧経路キーワードの残置確認は実装経路と import を分けて確認（`client_datetime_change` / `suggestClientDatetimeChangeAdjustmentsAction` / `ShiftAdjustmentDialog`）。
- 検証コマンドは `pnpm test:ut --run` と `pnpm typecheck` を実施。
