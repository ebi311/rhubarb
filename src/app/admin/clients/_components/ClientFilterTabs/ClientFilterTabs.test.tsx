...
describe('ClientFilterTabs', () => {
	...
	it('activeFilterに応じてアクティブなタブが表示される', () => {
		const { rerender } = render(<ClientFilterTabs activeFilter="all" onFilterChange={vi.fn()} />);
		expect(screen.getByRole('tab', { name: '全て' })).toHaveClass('tab-active');

		rerender(<ClientFilterTabs activeFilter="active" onFilterChange={vi.fn()} />);
		expect(screen.getByRole('tab', { name: '契約中' })).toHaveClass('tab-active');

		rerender(<ClientFilterTabs activeFilter="suspended" onFilterChange={vi.fn()} />);
		expect(screen.getByRole('tab', { name: '中断中' })).toHaveClass('tab-active');
	});

	it('タブクリックでonFilterChangeが呼ばれる', async () => {
		const user = userEvent.setup();
		const handleFilterChange = vi.fn();
		render(<ClientFilterTabs activeFilter="all" onFilterChange={handleFilterChange} />);
	...
	it('アクティブなタブをクリックしてもonFilterChangeは呼ばれる', async () => {
		const user = userEvent.setup();
		const handleFilterChange = vi.fn();
		render(<ClientFilterTabs activeFilter="active" onFilterChange={handleFilterChange} />);
