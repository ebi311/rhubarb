...
vi.mock('@/backend/services/serviceUserService', async () => {
	const actual = await vi.importActual<
		typeof import('@/backend/services/serviceUserService')
...
describe('createServiceUserAction', () => {
	...
	it('管理者以外は403', async () => {
		...
		mockService.createServiceUser.mockRejectedValue(
			new ServiceError(403, 'Forbidden'),
		);

		const result = await createServiceUserAction({
			name: '山田',
			address: '東京',
		});
	...
	it('バリデーションエラーは400', async () => {
		...
		mockService.createServiceUser.mockRejectedValue(
			new ServiceError(400, 'Validation error', [1]),
		);
...
describe('updateServiceUserAction', () => {
	it('未認証は401', async () => {
		...
		const result = await updateServiceUserAction('client-1', {
			name: '',
			address: '',
		});
		expect(result.status).toBe(401);
	...
	it('管理者以外は403', async () => {
		...
		mockService.updateServiceUser.mockRejectedValue(
			new ServiceError(403, 'Forbidden'),
		);
	...
	it('正常更新できる', async () => {
		...
		expect(mockService.updateServiceUser).toHaveBeenCalledWith(
			'user-1',
			'client-1',
			{
				name: '更新',
...
describe('suspendServiceUserAction', () => {
	it('管理者以外は403', async () => {
		...
		mockService.suspendServiceUser.mockRejectedValue(
			new ServiceError(403, 'Forbidden'),
		);
	...
	it('正常に中断できる', async () => {
		...
		expect(mockService.suspendServiceUser).toHaveBeenCalledWith(
			'user-1',
			'client-1',
		);
		expect(result.data).toEqual(mockClient);
...
describe('resumeServiceUserAction', () => {
	it('正常に再開できる', async () => {
		...
		expect(mockService.resumeServiceUser).toHaveBeenCalledWith(
			'user-1',
			'client-1',
		);
		expect(result.data).toEqual(mockClient);
