...
import { ServiceUserService, ServiceError } from "@/backend/services/serviceUserService";
...
describe("getServiceUsersAction", () => {
  ...
  it("不正なstatusは400", async () => {
    mockSupabase.auth.getUser.mockResolvedValue({ data: { user: { id: "user-1" } }, error: null });
    mockService.getServiceUsers.mockRejectedValue(new ServiceError(400, "Invalid status parameter"));
