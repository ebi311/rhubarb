...
describe("PATCH /api/clients/[id]/suspend", () => {
  const mockSupabase = {
    auth: {
      getUser: vi.fn(),
    },
  ...
  it("未認証の場合は401を返す", async () => {
    ...
    const request = new NextRequest(
      "http://localhost:3000/api/clients/client-1/suspend",
      { method: "PATCH" }
    );
  ...
  it("利用者が見つからない場合は404を返す", async () => {
    ...
    const request = new NextRequest(
      "http://localhost:3000/api/clients/client-999/suspend",
      { method: "PATCH" }
    );
  ...
  it("契約を正常に中断できる", async () => {
    ...
    mockRepository.suspend.mockResolvedValue(mockSuspendedClient);
    ...
    expect(NextResponse.json).toHaveBeenCalledWith({ data: mockSuspendedClient, error: null });
  });

  it("既に中断済みの契約も正常に処理できる（冪等性）", async () => {
    mockSupabase.auth.getUser.mockResolvedValue({
      data: { user: { id: "user-1" } },
      error: null,
    });
    ...
    mockRepository.findById.mockResolvedValue({
      ...
      name: "山田太郎",
      address: "東京都",
      contract_status: "suspended", // 既に中断済み
      office_id: "office-1",
      created_at: new Date(),
      updated_at: new Date(),
    });
